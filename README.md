...
2. **plan** — `terraform plan`, plan output saved as an artifact (runs for both PRs and pushes to `main`)
3. **apply** — runs only on push to `main` (or manually via `workflow_dispatch: action=apply`):
...
4. **destroy** — runs only manually via `workflow_dispatch: action=destroy`; runs `terraform destroy -auto-approve` and sends a Slack notification with the result status
